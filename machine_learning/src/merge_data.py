...
# Fungsi untuk menghitung jarak antara dua titik berdasarkan Haversine Formula
...
def calculate_matching_score(donor, receiver):
    ...
    # 1. makanan_disumbangkan → makanan_dibutuhkan
    ...
    # 2. kondisi_makanan → kondisi_makanan_diterima
    ...
    # 3. jumlah_disumbangkan → jumlah_dibutuhkan
    ...
    if donor['is_halal_donor'] == receiver['is_halal_receiver']:
        ...
        score += 0  # Tidak cocok secara mutlak
    
    # 5. is_for_child → is_for_child dan is_for_elderly → is_for_elderly
...
def merge_and_calculate_scores(donor_df, receiver_df, threshold=0.7):
    ...
    for _, donor in donor_df.iterrows():
        for _, receiver in receiver_df.iterrows():
            ...
            # Gabungkan data donor dan penerima serta tambahkan skor
            combined_entry = {
